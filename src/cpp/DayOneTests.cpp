...
namespace AdventOfCode {
    TEST(DayOne, puzzleOneAnswer) {
        const auto input = DayOneParser::parseFile(AdventOfCode::kDayOneInputFilePath);
        EXPECT_EQ(69528, DayOne::puzzleOneSolution(input));
    }

    TEST(DayOne, puzzleTwoAnswer) {
        const auto realInput = DayOneParser::parseFile(AdventOfCode::kDayOneInputFilePath);
